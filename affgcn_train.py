...
from Models import AFFGCN #模型
from Utils import AFFGCNDatesetReader,AFFGCNBatchIteraotr
...
class Instructor:
    def __init__(self,opt):
        self.opt=opt #配置
        dataset=AFFGCNDatesetReader(dataset=opt.dataset, embed_dim=opt.embed_dim,max_seq_len = opt.max_seq_len)
        self.train_data_loader = AFFGCNBatchIteraotr(data=dataset.semeval.train_data, batch_size=opt.batch_size, shuffle=True)
        self.test_data_loader = AFFGCNBatchIteraotr(data=dataset.semeval.test_data, batch_size=opt.batch_size, shuffle=False)
        self.model = opt.model_class(dataset.semeval.embedding_matrix, opt).to(opt.device)
    ...
    def run(self,repeats=3):
        ...
        _params=filter(lambda  p: p.requires_grad, self.model.parameters()) #构建一个新的迭代器
        ...
        for i in range(repeats):
            ...
            f_out.write('\nrepeat: '+str(i+1))
            self._reset_params()
            max_test_acc, max_test_f1 ,max_test_precision , max_test_recall= self._train(loss_function, optimizer)
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            for i_batch, sample_batched in enumerate(self.train_data_loader):
                ...
                if global_step % self.opt.log_step==0:
                    ...
                    if test_f1 > max_test_f1:  #f1是核心指标
...
def main():
    ...
    parser.add_argument('--model_name', default='affgcn', type=str,help="模型名称")
    parser.add_argument('--embed_dim', default=300, type=int,help="词向量维度")
    parser.add_argument('--batch_size', default=16, type=int,help="batch大小")
    parser.add_argument('--hidden_dim', default=300, type=int, help="LSTM的Hidden维度")
    ...
    parser.add_argument('--save', default=True, type=bool,help="保存？")
